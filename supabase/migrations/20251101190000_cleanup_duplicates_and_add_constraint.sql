-- Step 1: Remove duplicate rows, keeping only the first occurrence
WITH duplicates AS (
  SELECT id,
    ROW_NUMBER() OVER (
      PARTITION BY faculty_id, date, start_time, end_time 
      ORDER BY created_at ASC, id ASC
    ) as row_num
  FROM public.faculty_slots
)
DELETE FROM public.faculty_slots
WHERE id IN (
  SELECT id FROM duplicates WHERE row_num > 1
);

-- Step 2: Drop any existing unique constraint if it exists
DO $$ 
DECLARE
...
-- Step 4: Update trigger function to handle the constraint properly
